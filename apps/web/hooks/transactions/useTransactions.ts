...
export function useTransactions(initial?: Partial<Filters>) {
    ...
    (async () => {
      try {
        ...
        const [accsRes, catsRes] = await Promise.all([
          fetch(new URL("/api/accounts?user_id=1", base).toString(), { cache: 'no-store' }),
          fetch(new URL("/api/categories?user_id=1", base).toString(), { cache: 'no-store' }),
        ]);
        if (!accsRes.ok) throw new Error(await accsRes.text());
        if (!catsRes.ok) throw new Error(await catsRes.text());
        const [accs, cats] = await Promise.all([accsRes.json(), catsRes.json()]);
        if (!ignore) { setAccounts(accs); setCategories(cats); }
      } catch (e) {
  ...
  return { filters, setFilters, txns, total, accounts, categories, loading, error };
}
